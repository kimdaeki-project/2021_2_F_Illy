...
<c:if test="${!empty list}">												
 		<span class="pick_list_num">취소/반품/교환 내역 총 <strong>${count}</strong> 건</span>
		<div class="myPage_type_table">
			<table>
			<colgroup>
				<col style="width:15%"> <!-- 날짜/주문번호 -->
								...
								<tr>
									<td rowspan="${list.carts.size()}" class="order_day_num">
										...
										<c:if test="${carts.cart_state eq 'payment'}">
											<div class="btn_claim">
												<span class="btn_order_cancel" data-payment-id="${list.payment_id}">
													<a>주문취소</a>
												</span>
											</div>
										</c:if>
