...
public class Pager {
	...
	public void makeNum(Long totalCount) { //totalCount = 데이터의 총 갯수 
		//1. 전체 페이지 수 구하기 
		Long totalPage = totalCount/this.getPerPage();   //totalCount를 '한페이지당 뿌려줄 데이터 개수' 로 나누기
		...
		Long perBlock = 5L;   //일단 5개씩으로 설정 
		Long totalBlock = totalPage/perBlock; 
		if(totalPage%perBlock != 0){
			totalBlock++; 
		}
		//3.pn으로 현재 블럭 번호 구하기 
		Long curBlock = this.getPn()/perBlock; //현재 블럭은 지금 페이지 나누기 설정된 블럭단위의 몫 
